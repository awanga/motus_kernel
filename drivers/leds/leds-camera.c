...
#define MOTUS_LED_GPIO_PULL       GPIO_CFG_NO_PULL
...
#define ZEPPELIN_LED_GPIO_PULL    GPIO_CFG_PULL_DOWN
...
/* board-mot-pmic.c */
extern int mot_set_led_intensity(const enum ledtype type, int val);
extern int mot_flash_led_set_current(const uint16_t milliamps);
...
static int cam_led_do_torch_level(unsigned value)
{
   ...
   // set the current level in the PMIC LCD output used for torch/flashlight
    ...
    ret = mot_set_led_intensity(LED_LCD, level);
    if (ret) {
        printk(KERN_ERR "%s: mot_set_led_intensity() failed, error %d\n",
               __FUNCTION__, ret);
    } else {
...
static int cam_led_do_burst_level(unsigned value)
{
    ...
    int high_curr, low_curr = 0, total_curr, adj_curr, level = 0;
    int ret = 0;
    ...
    if (value == 0) {
        ...
        if (ret) {
            printk(KERN_ERR "%s: mot_flash_led_set_current() off failed, error %d\n",
                   __FUNCTION__, ret);
            // Re-attempt to disable the flash LED burst current in the PMIC
            ret |= mot_flash_led_set_current(0);
            if (ret) {
                printk(KERN_ERR "%s: mot_flash_led_set_current() re-try failed, error %d\n", 
                       __FUNCTION__, ret);
            }
        }
        // Disable the flash LED torch current in the PMIC
        ret = mot_set_led_intensity(LED_LCD, 0);
        if (ret) {
            printk(KERN_ERR "%s: mot_set_led_intensity() off failed, error %d\n",
                   __FUNCTION__, ret);
 
            // Re-attempt to disable the flash LED torch current in the PMIC
            ret = mot_set_led_intensity(LED_LCD, 0);
            if (ret) {
                printk(KERN_ERR "%s: mot_set_led_intensity() re-try failed, error %d\n",
                       __FUNCTION__, ret);
            }
        }
        return ret;
    ...
    ret |= mot_flash_led_set_current(high_curr);
    if (ret) {
    ...
    ret = mot_set_led_intensity(LED_LCD, level);
    if (ret) {
        printk(KERN_ERR "%s: mot_set_led_intensity() on failed, error %d\n",
               __FUNCTION__, ret);
    }
...
static int cam_led_suspend (struct platform_device *pdev, pm_message_t mesg)
{
    ...
    ret = mot_set_led_intensity(LED_LCD, 0);
    if (ret) {
        printk (KERN_ERR "%s: mot_set_led_intensity(0) failed, error %d\n",
                 __FUNCTION__, ret);
        return ret;
    }
    // update the current brightness level
...
static int cam_led_probe(struct platform_device *pdev)
{
    ...
    gpio_tlmm_config(GPIO_CFG(camera_led_gpio, 0, GPIO_CFG_OUTPUT, 
                              led_gpio_pull, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
