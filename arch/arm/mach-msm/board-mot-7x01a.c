...
static void sdcc_gpio_init(void)
{
    ...
    rc = gpio_tlmm_config(GPIO_CFG(SD_DETECT_N_SIGNAL, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
...
static unsigned sdcc_cfg_data[][7] = {
	...
	GPIO_CFG(WLAN_HOST_IRQ , 0, GPIO_CFG_INPUT,  GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
	},
};
...
{
	msm_add_sdcc(2, &mot_7x01_sdcc2_data); /* BRCM 4325 */
}
...
static struct msm_camera_sensor_flash_data flash_mot = {
...
void __init morrison_setup(void)
{
    ...
    if (mot_hw_rev < 0x20) /* legacy HW */
    {
        ...
        gpio_tlmm_config(GPIO_CFG(18, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);    /* HAPTICS_PWM */
        gpio_tlmm_config(GPIO_CFG(21, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);     /* SEND_END_DET */
        gpio_tlmm_config(GPIO_CFG(27, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);     /* QWERTY_INT */
        gpio_tlmm_config(GPIO_CFG(28, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);   /* ACCEL_INT */
        gpio_tlmm_config(GPIO_CFG(41, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);     /* SLIDER_INT_N */
        gpio_tlmm_config(GPIO_CFG(77, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);    /* EL_PWM */
        gpio_tlmm_config(GPIO_CFG(84, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);    /* HEAD_SEL_N */
    }
    else if (mot_hw_rev >= 0x2D)    /* P2B */
    {
        gpio_request(100, "fairchild_led_blinker");
        gpio_tlmm_config(GPIO_CFG(100, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);    /* HEAD_SEL_N */
    ...
    if (mot_hw_rev >= 0x20)
    {
        gpio_tlmm_config(GPIO_CFG(SFH7743_GPIO_INTR, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);     /* proximity */
    }

    if (mot_hw_rev >= 0x30) /* P3A runs spurious interrupts without pullup */
    {
        gpio_tlmm_config(GPIO_CFG(SFH7743_GPIO_INTR, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);     /* proximity */
    }
...
void __init motus_setup(void)
{
    ...
    gpio_tlmm_config(GPIO_CFG(20, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);    /* FLIP_OPEN_B */
    gpio_tlmm_config(GPIO_CFG(83, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);    /* FLASH_CAM_KEY */
...
void __init zeppelin_setup(void)
{
    ...
    gpio_tlmm_config(GPIO_CFG(28, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
