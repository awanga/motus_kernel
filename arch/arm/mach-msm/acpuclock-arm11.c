...
static int acpuclk_arm11_set_rate(int cpu, unsigned long rate, enum setrate_reason reason);
static unsigned long acpuclk_arm11_get_rate(int cpu);
...
{
	int ret = acpuclk_arm11_get_rate(smp_processor_id());
	acpuclk_arm11_set_rate(smp_processor_id(), POWER_COLLAPSE_KHZ, SETRATE_PC);
	return ret;
}*/
...
{
	int ret = acpuclk_arm11_get_rate(smp_processor_id());
	acpuclk_arm11_set_rate(smp_processor_id(), WAIT_FOR_IRQ_KHZ, SETRATE_SWFI);
	return ret;
}*/
...
static struct acpuclk_data acpuclk_arm11_data = {
        .set_rate = acpuclk_arm11_set_rate,
        .get_rate = acpuclk_arm11_get_rate,
        .power_collapse_khz = POWER_COLLAPSE_KHZ,
	.wait_for_irq_khz = POWER_COLLAPSE_KHZ,
        .switch_time_us = 50,
};

static int __init acpuclk_arm11_init(struct acpuclk_soc_data *soc_data)
{
...
#endif
	acpuclk_register(&acpuclk_arm11_data);

	return 0;
}

struct acpuclk_soc_data acpuclk_arm11_soc_data __initdata = {
        .max_speed_delta_khz = 400000,
        .init = acpuclk_arm11_init,
};
